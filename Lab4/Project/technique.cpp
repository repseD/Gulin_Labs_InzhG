...
// Деструктор
Technique::~Technique() {
    // Проходится по списку
    for (ShaderObjList::iterator it = m_shaderObjList.begin(); it != m_shaderObjList.end(); it++) {
        glDeleteShader(*it); // Удаляет шейдеры
    }

    // Если есть программа, она удаляется и счётчик обнуляется
    if (m_shaderProg != 0) {
...
bool Technique::Finalize() {
    ...
    glLinkProgram(m_shaderProg); // Линковка
